...
impl BotEventChannel {
    pub fn from_wire(text: impl Into<String>) -> Self {
        let text = text.into();
        if let Some(hub) = HubChannel::from_wire(&text) {
            return Self::Hub(hub);
        }
        if text.starts_with("hub:") {
            return Self::HubUnknown(HubUnknownChannel(text));
        }
        Self::Upstream(UpstreamChannel(text))
    }
    pub fn as_str(&self) -> &str {
        match self {
            Self::Hub(hub) => hub.as_str(),
            Self::HubUnknown(text) => text.as_str(),
            Self::Upstream(text) => text.as_str(),
        }
    }
}
