//! SSH wrap tip: over SSH without `open-grok wrap`, advertise that wrapping the
...
/// Ephemeral-tip dedup key for the SSH `open-grok wrap` hint.
...
/// Build "Over SSH? Run `open-grok wrap ssh <host>` locally for clipboard +
    ...
    EphemeralTip {
        ...
        ..EphemeralTip::new(
            ...
            Line::from(vec![
                ...
                Span::styled("open-grok wrap ssh <host>", command),
...
mod tests {
    ...
    fn ssh_wrap_tip_points_to_doctor() {
        ...
        assert_eq!(
            text,
            "Over SSH? Run open-grok wrap ssh <host> locally for clipboard + terminal restore"
        );
    }
