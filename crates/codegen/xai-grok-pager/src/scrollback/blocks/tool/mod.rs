...
macro_rules! delegate_tool {
    ($self:expr, $method:ident ( $($arg:expr),* )) => {
        match $self {
            ...
            ToolCallBlock::SentMessage(block) => block.$method($($arg),*),
...
impl ToolCallBlock {
    ...
    pub fn is_success(&self) -> bool {
        match self {
            ...
            ToolCallBlock::SentMessage(block) => block.is_success(),
    ...
    pub fn set_started_at(&mut self, instant: std::time::Instant) {
        match self {
            ...
            ToolCallBlock::SentMessage(block) => block.started_at = Some(instant),
    ...
    pub fn start_timing(&mut self) {
        match self {
            ...
            ToolCallBlock::SentMessage(block) => {
                if block.started_at.is_none() {
    ...
    pub(crate) fn searchable_text(&self) -> Option<String> {
        match self {
            ...
            ToolCallBlock::SentMessage(block) => block.searchable_text(),
