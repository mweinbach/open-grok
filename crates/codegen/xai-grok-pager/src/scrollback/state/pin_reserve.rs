...
impl ScrollbackState {
    ...
    pub(super) fn pin_reserve_prompt_scroll_target(&self) -> Option<usize> {
        ...
        let prompt_y = *cache.virtual_y.get(idx)?;
        let entry_y = prompt_y.saturating_sub(base);
...
mod tests {
    ...
    fn tall_history_then_prompt() -> (ScrollbackState, usize) {
        ...
        for index in 0..30 {
            state.push_block(agent_block(&format!("filler line {index}")));
        }
