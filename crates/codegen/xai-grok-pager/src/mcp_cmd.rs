...
mod tests {
    ...
    fn grok_com_known_only_with_toml_definition() {
        // Unique name: `grok_home()` is process-wide OnceLock, so OPENGROK_HOME
        // EnvGuard is a no-op if another test already resolved it. A leftover
        // `grok_com_*` in the real ~/.opengrok disabled list would fail an orphan
        ...
        std::fs::create_dir_all(defined.path().join(".opengrok")).unwrap();
        std::fs::write(
            defined.path().join(".opengrok").join("config.toml"),
            format!(
                r#"
[mcp_servers.{name}]
...
"#
    ...
    fn enable_and_disable_parse_name() {
        ...
        let args = PagerArgs::try_parse_from(["open-grok", "mcp", "disable", "user-slack"])
