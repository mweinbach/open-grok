...
impl CommandRegistry {
    ...
    /// Show or hide the `/usage` command (combined xAI + Codex billing
    /// surface gate). Driven from
    /// [`crate::slash::SlashController::set_billing_surface_visible`] so the
    /// command disappears entirely when no provider can answer it, while the
    /// separate `restricted` deny list still fail-closes a tier-gated
    /// `/usage` even after this reveals it.
