...
impl TitleManager {
    ...
    pub fn update(&mut self, state: &TitleState<'_>) -> Option<String> {
        ...
        if !has_parts {
            self.composed.clear();
            self.composed.push_str("Open Grok");
        }
    ...
    pub fn reset(&mut self) -> String {
        let esc = build_title_escape("Open Grok");
        self.last_title.clear();
        self.last_title.push_str("Open Grok");
...
fn write_item(
    ...
    match item {
        TitleItem::Grok => {
            ...
            // Existing configs may still use the `grok` serde alias, but new
            // templates serialize this item as `open-grok`.
            buf.push_str("Open Grok");
        }
        TitleItem::Spinner => {
...
mod tests {
    ...
    fn spinner_only_shown_when_active() {
        ...
        mgr.update(&state);
        assert!(mgr.last_title.contains(" - Open Grok"));
    ...
    fn activity_shows_waiting_when_busy_without_activity() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Waiting - Open Grok");
    }
    ...
    fn model_item_shown_when_present() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "grok-3 - Open Grok");
    }
    ...
    fn turn_timer_shown_when_above_one_second() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "42s - Open Grok");
    }
    ...
    fn reset_clears_state_and_emits_grok() {
        ...
        mgr.update(&state);
        assert_ne!(mgr.last_title, "Open Grok");
    ...
    fn default_config_active_turn_with_permissions() {
        ...
        for t in [&t1, &t2] {
            assert!(t.contains("Open Grok"), "title missing 'Open Grok': {t}");
    ...
    fn all_items_present_in_order() {
        ...
        assert_eq!(
            mgr.last_title,
            "Thinking - proj - grok-3 - workspace - Open Grok"
        );
    }
