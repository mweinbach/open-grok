        ...
        DashboardRowId::TopLevel(id) => {
            ...
            let (question, options, request_id, reject_option) =
                if let Some(p) = agent.permission_queue.front() {
                    ...
                    let selected_words = p.bash_highlights.as_ref().map(|highlights| {
                    ...
                    let deny_selected_words = p
                        ...
                        .map(|highlights| {
                            highlights.highlighted_words[..p.bash_deny_selection_count].join(" ")
                        });
                    let opts = p
                        .options
                        .iter()
...
mod tests {
    ...
    #[test]
    fn sent_message_tool_presentations_keep_message_response_type() {
        use crate::scrollback::block::RenderBlock;
        use crate::scrollback::blocks::ToolCallBlock;
        use crate::scrollback::blocks::tool::{SentMessagePresentation, SentMessageToolCallBlock};
        ...
        for presentation in [
            ...
            let mut agent = crate::test_util::make_agent_view(Some("parent"), "/tmp");
            agent
                .scrollback
                .push(ScrollbackEntry::new(RenderBlock::ToolCall(
                    ToolCallBlock::SentMessage(SentMessageToolCallBlock::new(
                        presentation,
                        Some("child".into()),
                        Some("Continue the task".into()),
                    )),
                )));
            assert_eq!(extract_last_response_type(&agent), "Message");
        }
    }
