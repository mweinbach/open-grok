...
pub(super) const WORKFLOWS_EMPTY_PLACEHOLDER: &str =
    "No workflows available. Ask Open Grok to help make you one!";
...
pub(super) fn build_workflows_picker_rows(
    ...
    let mut visible: Vec<&WorkflowInfo> = workflows
        .iter()
        .filter(|workflow| {
            fuzzy_matches(&workflow.name, query) || fuzzy_matches(&workflow.description, query)
        })
        .collect();
    visible.sort_by(|left, right| cmp_str_ci(&left.name, &right.name));
    visible
        .into_iter()
        .map(|wf| {
            let mut fields = Vec::new();
            if let Some(ref workflow_path) = wf.path {
                fields.push(("path".to_string(), workflow_path.clone()));
            }
    ...
    fn filters_on_name_and_description() {
        let workflows = TabDataState::Loaded(vec![
            WorkflowInfo {
                ...
                when_to_use: None,
                source: "user".into(),
                path: Some("/home/u/.opengrok/workflows/alpha-wf.rhai".into()),
            },
            WorkflowInfo {
                name: "beta-wf".into(),
        ...
        assert_eq!(
            ...
            [(
                "path".to_string(),
                "/home/u/.opengrok/workflows/alpha-wf.rhai".to_string()
            )]
        );
