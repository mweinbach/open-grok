...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Open Grok** VERSION+channel (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | channel (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **Open Grok**  VERSION (left-aligned).
    HeroInline,
}
...
mod tests {
    ...
    fn version_badge_carries_no_release_label() {
        ...
        assert!(full.contains("Open Grok"), "full badge: {full:?}");
        assert!(inline.contains("Open Grok"), "inline badge: {inline:?}");
