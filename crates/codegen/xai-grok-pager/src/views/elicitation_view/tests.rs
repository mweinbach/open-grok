...
fn buffer_text(buf: &Buffer) -> String {
    ...
    for row in 0..area.height {
        for column in 0..area.width {
            if let Some(cell) = buf.cell((column, row)) {
...
fn render_to_text(state: &mut ElicitationViewState, width: u16, height: u16) -> String {
        ...
        for column in 0..area.width {
            if let Some(cell) = buf.cell((column, row)) {
                assert!(
                    ...
                    "control char leaked into cell ({column},{row}): {:?}",
                    cell.symbol()
                );
            }
        }
...
fn append_char_rejects_control_and_unsafe_chars() {
    let mut state = ElicitationViewState::from_request(form_req(), None, None);
    for character in "a\x1b@\u{202E}b\r".chars() {
...
fn many_field_req(count: usize) -> McpElicitExtRequest {
    let mut properties = serde_json::Map::new();
    let mut required = Vec::new();
    for index in 0..count {
        properties.insert(format!("field{index:02}"), json!({ "type": "string" }));
        required.push(format!("field{index:02}"));
    }
    let mut req = form_req();
    req.mode = McpElicitModeFields::Form {
        requested_schema: Some(json!({
...
fn tall_form_keeps_actions_visible_and_follows_cursor() {
    ...
    let screen_h = 24u16;
    let height = elicitation_view_height(&state, screen_h, 75);
    assert!(height <= screen_h, "card must not exceed the screen");

    let text = render_to_text(&mut state, 80, height);
...
fn failed_accept_error_is_visible_on_clipped_form() {
    ...
    let height = elicitation_view_height(&state, 24, 75);
    let text = render_to_text(&mut state, 80, height);
...
fn url_consent_shows_host_and_full_url() {
    ...
    let height = elicitation_view_height(&state, 60, 75);
    let text = render_to_text(&mut state, 80, height);
...
fn url_tail_reachable_by_scrolling_on_short_terminal() {
    ...
    let mut state = ElicitationViewState::from_request(url_req(&url), None, None);
    let height = elicitation_view_height(&state, 14, 75);
    let text = render_to_text(&mut state, 80, height);
    assert!(
        !text.contains("tail-end-marker"),
...
fn long_text_draft_is_reviewable_in_full() {
    ...
    let height = elicitation_view_height(&state, 40, 75);
    let text = render_to_text(&mut state, 80, height);
    assert!(
