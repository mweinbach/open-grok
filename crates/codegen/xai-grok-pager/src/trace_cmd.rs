...
pub struct TraceArgs {
    ...
    #[arg(long)]
    pub local: bool,
    /// Output path (default: $OPENGROK_HOME/trace-exports/<session-id>.tar.gz)
    #[arg(short, long)]
...
pub fn build_session_tar(
    ...
    let mut file_count: u32 = 0;
    {
        ...
        let metadata = ExportMetadata {
            ...
            grok_version: xai_grok_version::version_with_commit().to_owned(),
...
async fn run_upload(
    ...
    let upload_method = match upload_method {
        ...
        None => {
            ...
            anyhow::bail!(
                "No upload credentials. Run `open-grok login` or set a deployment key. \
...
impl UploadAttempt<'_> {
    ...
    pub fn handle_failure(&self, error: &anyhow::Error) -> anyhow::Error {
        ...
        if self.json {
            ...
            eprintln!("  Retry:  open-grok trace {}", self.session_id);
