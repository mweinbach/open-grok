#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    #[test]
    fn retry_exhausted_rate_limited_server_detail_stays_auth_aware() {
        use xai_grok_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_OAUTH;
        ...
        match last_session_event(&scrollback) {
            ...
            other => panic!("expected auth-aware RetryFailed, got {other:?}"),
        }
    }
    ...
    /// Legacy WebLogin auth keeps its verbose message (with `open-grok logout` /
    /// `open-grok login` guidance), not the generic re-auth prompt.
    #[test]
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) ... deprecated authentication method (WebLogin) ... \
                          run `open-grok logout` then `open-grok login`"
                    .into(),
            },
            &mut session,
            &mut scrollback, false);
