#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.opengrok` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.opengrok` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.opengrok` reads).
    #[test]
