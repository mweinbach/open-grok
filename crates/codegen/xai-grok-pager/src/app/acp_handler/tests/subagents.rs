#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn replaying_a_spawn_rebuilds_a_removed_row_and_keeps_the_finish() {
        ...
        assert!(!handle_ext_notification(&spawn, &mut app), "loading replay suppresses intermediate redraw");
        ...
        assert!(!handle_ext_notification(&finish, &mut app), "loading replay suppresses intermediate redraw");
    ...
    fn open_subagent_fullscreen_replays_when_flag_false_and_prompt_only() {
        with_replay_disk_home(|_| {
            ...
            if let Some(info) = agent.subagent_sessions.get_mut(child_sid) {
                info.transcript = Default::default();
            }
