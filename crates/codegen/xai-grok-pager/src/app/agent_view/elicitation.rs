...
impl AgentView {
    pub(super) fn handle_elicitation_key(&mut self, key: &KeyEvent) -> InputOutcome {
        ...
        if ev.focus == ElicitationFocus::Editing {
            let multi = ev
                .current_field()
                .is_some_and(|field| field.is_multi_select());
            if multi {
                match key.code {
        ...
        if let Some(walk) = RowWalk::from_key(key) {
            ev.move_focus(matches!(walk, RowWalk::Forward), true);
            return InputOutcome::Changed;
        }

        match key.code {
            KeyCode::Up | KeyCode::Char('k') => {
                ev.move_focus(false, false);
                InputOutcome::Changed
            }
            KeyCode::Down | KeyCode::Char('j') => {
                ev.move_focus(true, false);
                InputOutcome::Changed
            }
            ...
            {
                if let Some(idx) = crate::views::question_view::option_index_for_key(character)
                    && idx < ev.field_count()
                {
                    ev.focus = ElicitationFocus::Fields;
    ...
    pub(super) fn handle_elicitation_paste(&mut self, text: &str) -> InputOutcome {
        ...
        let editing_text = ev.focus == ElicitationFocus::Editing
            && ev.current_field().is_some_and(|field| field.is_text());
