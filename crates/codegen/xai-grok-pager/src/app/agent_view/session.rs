...
impl AgentView {
    ...
    pub fn new(session: AgentSession, scrollback: ScrollbackState) -> Self {
        ...
        let mut view = Self {
            ...
            inline_media_load_failed: Default::default(),
    ...
    fn apply_reload_outcome(&mut self, reload: SessionReload, success: bool) -> bool {
        ...
        if success && reload.saw_replay {
            ...
            let mut tail = std::mem::replace(&mut self.scrollback, reload.stash.scrollback);
            ...
            self.workflow_blocks.extend(reload.stash.workflow_blocks);
            {
                let mut live_by_id: HashMap<String, _> = std::mem::take(&mut self.workflow_runs)
                    ...
                    .collect();
                let mut merged =
                    Vec::with_capacity(reload.stash.workflow_runs.len() + live_by_id.len());
                for run in reload.stash.workflow_runs {
                ...
                self.cleared_workflow_runs
                    .extend(reload.stash.cleared_workflow_runs);
                merged.retain(|run| !self.cleared_workflow_runs.contains(&run.run_id));
                self.workflow_runs = merged;
            }
            for (run_id, rev) in reload.stash.workflow_run_revisions {
            ...
            if !reload.saw_todo_update {
                self.todo = reload.stash.todo;
            }
            dropped_heavy = false;
        } else {
            let floor = self.scrollback.id_floor();
            let staging_generations = self.scrollback.invalidation_generations();
            self.scrollback = reload.stash.scrollback;
            self.scrollback.raise_id_floor(floor);
            self.scrollback
                .raise_invalidation_floor(staging_generations);
            self.session.tracker = reload.stash.tracker;
            self.todo = reload.stash.todo;
            self.workflow_blocks = reload.stash.workflow_blocks;
            self.workflow_runs = reload.stash.workflow_runs;
            self.workflow_run_revisions = reload.stash.workflow_run_revisions;
            self.cleared_workflow_runs = reload.stash.cleared_workflow_runs;
