...
/// keeps the scrollback flow: session token/cost, then the combined xAI +
/// Codex usage block.
///
/// In minimal mode, emits the per-session token/cost fetch first, then chains
/// the combined consumer usage block so layout stays ordered. `usage_visible`
/// (the xAI billing/UI gate) only controls the xAI half; OpenAI Codex quota
/// loads independently whenever a Codex account is connected.
