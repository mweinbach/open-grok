...
mod tests {
    ...
    #[test]
    fn metadata_preserves_selected_model_id_without_catalog_inference() {
        let mut agent = eligible_agent();
        assert!(agent_metadata(&agent).model.is_none());
        let model_id = acp::ModelId::new("provider-local/model-route");
        agent.session.models.current = Some(model_id.clone());
        assert_eq!(
            agent_metadata(&agent).model.as_deref(),
            Some("provider-local/model-route")
        );
        agent.session.models.available.insert(
            model_id.clone(),
            acp::ModelInfo::new(model_id, "Friendly display name"),
        );
        assert_eq!(
            agent_metadata(&agent).model.as_deref(),
            Some("provider-local/model-route")
