...
- Sessions save automatically. Resume the latest with `open-grok -c`, or pick
  one with `/resume` (`Ctrl+S`).
- Long session getting slow? `/compact` frees context; `/context` shows
  where it's going.
- Automate anything: `open-grok -p "summarize new TODOs" --output-format json`
  runs headless — great for scripts and CI.
- Stay current with `open-grok update`; see what changed with `/release-notes`.
