...
//! file logger that writes to `~/.opengrok/logs/memory.log`.
//!
//! ## When to use
//!
...
//! tail -f ~/.opengrok/logs/memory.log      # watch in another terminal
//! ```
...
mod inner {
    ...
    ///
    /// Writes to `~/.opengrok/logs/memory.log`. Filters to `xai_memory=trace`.
