...
pub fn record_join(span: &Region, outcome: &str, elapsed_ms: u64, notice_shown: bool) {
    let span_state = span.span();
    span_state.record("outcome", outcome);
    span_state.record("elapsed_ms", elapsed_ms);
    span_state.record("notice_shown", notice_shown);
}
...
mod tests {
    ...
    #[test]
    fn absent_phases_are_omitted_and_explicit_close_records_once() {
        let timer = SessionEndTimer::new_shared();
        ...
        timed_child(&timer, Phase::HooksStop, &tracing::Span::none()).close();
        timer.record(Phase::HooksStop, Duration::from_millis(42));
        timer.write_event_phases(&mut event);
        assert_eq!(
            serde_json::to_value(&event).unwrap(),
            serde_json::json!({"hooks_stop_ms": 42})
        );
    }

    #[test]
    fn dropped_phase_records_elapsed_without_explicit_close() {
        let timer = SessionEndTimer::new_shared();
        {
            let _phase = timed_child(&timer, Phase::WorkflowsPersist, &tracing::Span::none());
        }
        let mut event = crate::events::SessionEndTimings::default();
        timer.write_event_phases(&mut event);
        assert!(event.workflows_persist_ms.is_some());
    }
