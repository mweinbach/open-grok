...
pub const OUT_ENV: &str = "OPENGROK_SPAN_PROFILE_OUT";
...
impl tracing::field::Visit for NameVisitor {
    fn record_str(&mut self, field: &tracing::field::Field, value: &str) {
        if field.name() == "name" {
            self.0 = Some(profile_label(value));
        }
    }
...
fn profile_label(value: &str) -> String {
    crate::redact_common::redact_to_owned(&crate::redact_common::url_origin(value))
        .chars()
        .map(|character| {
        ...
        for ancestor in span.scope().from_root() {
            ...
            if ancestor.id() == id {
                ...
                let detail = extensions
                    .get::<SpanTiming>()
                    .and_then(|timing| timing.detail.as_deref());
...
pub(crate) mod test_support {
    ...
    pub(crate) fn folded_with_layer(field: impl FnOnce()) -> String {
...
mod tests {
    ...
    fn record_accumulates_per_path_and_holds_the_cap() {
        ...
        for index in 0..MAX_PATHS - 1 {
            profile.record(format!("p{index}"), 1);
        }
    ...
    #[test]
    fn profile_labels_do_not_leak_secrets_or_url_paths() {
        assert_eq!(
            profile_label("https://user:password@example.test/private?token=CANARY"),
            "https://example.test/"
        );
        assert!(!profile_label("sk-CANARYabcdefghij1234567890").contains("CANARY"));
        assert_eq!(profile_label("timer;forged\nframe"), "timer_forged_frame");
    }
}
