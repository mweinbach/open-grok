...
impl EventEnrichment {
    fn capture() -> Self {
        ...
        Self {
            entrypoint: identity.map(|index| index.entrypoint.as_str()),
            is_leader_mode: identity.map(|index| index.leader == LeaderMode::Attached),
            is_interactive: identity.map(|index| index.interactivity == Interactivity::Interactive),
            is_ci: *IS_CI.get_or_init(is_ci_env),
            release_channel: crate::process_info::release_channel().map(|context| context.as_str()),
            dev_build: !xai_grok_version::is_release_build(),
            os: std::env::consts::OS,
            arch: std::env::consts::ARCH,
            cpu_cores: process.cpu_cores,
            cpu_share_percent: process.cpu.map(|window| window.share_percent),
            cpu_window_ms: process.cpu.map(|window| window.window_ms),
            child_cpu_share_percent: process.cpu.and_then(|window| window.child_share_percent),
...
mod tests {
    ...
    #[test]
    fn dev_build_enrichment_uses_fork_build_identity() {
        let enrichment = serde_json::to_value(EventEnrichment::capture()).unwrap();
        assert_eq!(
            enrichment["dev_build"],
    ...
    fn reserved_event_keys_derive_from_the_serialized_schema() {
        ...
        let reserved: std::collections::BTreeSet<String> = RESERVED_EVENT_KEYS
            .iter()
