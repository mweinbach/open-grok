...
pub fn snapshot() -> ProcessMetrics {
    ...
    let first = FIRST_SNAPSHOT.get_or_init(|| FirstSnapshot {
        at: sample.now,
        process_age_secs: xai_tty_utils::process_start_time()
    ...
    ProcessMetrics {
        ...
        memory_limit_bytes: xai_tty_utils::process_memory_limit(),
        cpu_cores: *CPU_CORES.get_or_init(|| {
            std::thread::available_parallelism()
                .ok()
                .map(|count| usize::from(count) as u64)
        }),
        uptime_secs,
    }
}
