...
pub(crate) fn detect_plain_urls_with_offset(
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        for link in finder.links(&line_text) {
            ...
            let overlaps = existing.iter().chain(result.iter()).any(|hyperlink| {
                hyperlink.line_index == line_index
                    && col_start < hyperlink.column_range.end
                    && hyperlink.column_range.start < col_end
            });
...
mod tests {
    ...
    fn plain_email_in_prose_produces_mailto_target() {
        let hyperlinks = finish_and_get_hyperlinks("Email foo@bar.com please.\n");
    ...
    fn scp_git_remote_is_not_mailto() {
        ...
        assert!(hyperlinks.iter().all(|h| !h.url.starts_with("mailto:")));
    }
