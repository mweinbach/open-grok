...
impl Theme {
    ...
    /// One step fainter than [`Self::dim`]: `gray_dim` blended 0.66 toward
    /// `bg_base`, for text that must stay present but read after every label.
    /// Falls back to [`Self::dim`] on palettes that cannot blend.
