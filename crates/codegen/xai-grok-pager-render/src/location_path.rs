...
///
/// After a `~` / `$OPENGROK_HOME` / `$GROK_HOME` prefix (or a leading `/` /
/// drive letter / UNC `\\server\share` / `//host/share` / `\\?\UNC\server\share`),
/// the last two components stay full and earlier ones become one letter.
/// Leading dots are kept plus the first non-dot character (`.opengrok` → `.o`,
/// `..cache` → `..c`). Literal `.` / `..` stay as-is. Drive-relative `C:foo\bar`
/// does not gain a root separator; rooted `\foo\bar` keeps one. Paths with 0–2
/// components after the prefix are unchanged.
pub(crate) fn shorten_location_path(path: &str) -> Cow<'_, str> {
    const KEEP_FULL: usize = 2;
    const OPENGROK_HOME_PREFIX: &str = "$OPENGROK_HOME";
    ...
    let (prefix, remainder, restore_root) = if matches_home_prefix(path, OPENGROK_HOME_PREFIX) {
        (
            Cow::Borrowed(OPENGROK_HOME_PREFIX),
            path.get(OPENGROK_HOME_PREFIX.len()..)
                .unwrap_or("")
                .trim_start_matches(['/', '\\']),
            false,
        )
    } else if matches_home_prefix(path, GROK_HOME_PREFIX) {
        (
...
fn matches_home_prefix(path: &str, prefix: &str) -> bool {
    path == prefix
        || path
            .strip_prefix(prefix)
            .is_some_and(|rest| rest.starts_with(['/', '\\']))
