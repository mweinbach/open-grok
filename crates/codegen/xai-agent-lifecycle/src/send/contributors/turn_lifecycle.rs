...
#[cfg(test)]
mod policy_tests {
    use super::{AnalyticsClass, InputAuthority, QueuePolicy};

    #[test]
    fn protected_and_hidden_inputs_are_never_editable() {
        for (policy, visible, editable) in [
            (QueuePolicy::VisibleProtected, true, false),
            (QueuePolicy::VisibleEditable, true, true),
            (QueuePolicy::Hidden, false, false),
        ] {
            assert_eq!(policy.is_visible(), visible, "{policy:?}");
            assert_eq!(policy.is_editable(), editable, "{policy:?}");
        }
    }
    #[test]
