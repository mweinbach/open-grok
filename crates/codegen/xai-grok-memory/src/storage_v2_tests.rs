...
#[test]
fn v2_remember_does_not_touch_legacy_memory_tree() {
    let temp = TempDir::new().unwrap();
    let legacy = temp.path().join("memory");
    let v2 = temp.path().join("memory-v2");
    std::fs::create_dir_all(&legacy).unwrap();
    std::fs::write(legacy.join("MEMORY.md"), "# Legacy must stay untouched\n").unwrap();

    let storage =
        MemoryStorage::new_for_mode(Path::new("/home/user/project"), Some(&v2), MemoryMode::V2);
    storage.ensure_initialized().unwrap();
    storage
        .save_remember_note("Remember this isolated fact.")
        .unwrap();
    ...
    assert!(v2.join("global").is_dir());
    assert!(!v2.join("MEMORY.md").exists());
}
