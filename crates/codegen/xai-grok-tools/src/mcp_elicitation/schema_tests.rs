...
fn preserves_schema_property_order() {
    ...
    let names: Vec<&str> = specs.iter().map(|value_f| value_f.name.as_str()).collect();
...
fn builds_string_and_required() {
    ...
    let email = specs
        .iter()
        .find(|value_f| value_f.name == "email")
        .unwrap();
...
fn rejects_too_many_properties() {
    let mut properties = serde_json::Map::new();
    for value_i in 0..=MAX_ELICIT_FIELDS {
        properties.insert(format!("f{value_i}"), json!({ "type": "string" }));
    }
    let schema = json!({
        "type": "object",
        "properties": properties
...
fn string_default_uses_the_draft_cap() {
    ...
    assert_eq!(
        default.as_deref().map(|value_d| value_d.len()),
