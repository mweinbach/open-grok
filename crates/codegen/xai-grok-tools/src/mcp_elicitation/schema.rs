...
pub fn chars_within(value_s: &str, max: usize) -> bool {
    value_s.chars().count() <= max
}

pub fn take_chars(value_s: &str, max: usize) -> String {
    value_s.chars().take(max).collect()
}

fn schema_bytes_ok(schema: &Value) -> bool {
    serde_json::to_vec(schema)
        .map(|value_b| value_b.len() <= MAX_ELICIT_SCHEMA_BYTES)
        .unwrap_or(false)
}
...
pub fn parse_form_schema(schema: &Value) -> Result<Vec<ElicitFieldSpec>, String> {
    ...
    let type_ok = obj
        .get("type")
        .and_then(|value_t| value_t.as_str())
        .is_none_or(|value_t| value_t == "object");
    if !type_ok {
    ...
    let required: HashSet<String> = obj
        .get("required")
        .and_then(|value_r| value_r.as_array())
        .map(|arr| {
            arr.iter()
                .filter_map(|value_v| value_v.as_str().map(str::to_string))
                .collect()
        })
        .unwrap_or_default();
    ...
    let Some(props) = obj
        .get("properties")
        .and_then(|value_p| value_p.as_object())
    else {
...
fn field_from_schema(name: &str, prop: &Value, required: bool) -> Result<ElicitFieldSpec, String> {
    ...
    let description = prop.get("description").and_then(|value_d| value_d.as_str());
    if let Some(value_d) = description
        && !chars_within(value_d, MAX_ELICIT_DESC_CHARS)
    {
        return Err(format!(
    ...
    if let Some(Value::String(value_s)) = prop.get("default")
        && !chars_within(value_s, MAX_ELICIT_DRAFT_CHARS)
    {
        return Err(format!(
            "requestedSchema default exceeds {MAX_ELICIT_DRAFT_CHARS} characters"
        ));
    }
    let default_str = prop.get("default").map(|value_d| match value_d {
        Value::String(value_s) => value_s.clone(),
        Value::Number(value_n) => value_n.to_string(),
        Value::Bool(value_b) => value_b.to_string(),
...
fn field_kind_from_schema(
    ...
    if let Some(values) = prop.get("enum").and_then(|value_e| value_e.as_array()) {
        let names = prop.get("enumNames").and_then(|value_n| value_n.as_array());
        let options: Vec<ElicitOption> = values
            .iter()
            .enumerate()
            .filter_map(|(value_i, value_v)| {
                let value = json_scalar_to_string(value_v)?;
                let label = names
                    .and_then(|value_n| value_n.get(value_i))
                    .and_then(|value_l| value_l.as_str())
                    .map(str::to_string)
    ...
    if let Some(one_of) = prop.get("oneOf").and_then(|value_o| value_o.as_array()) {
    ...
    let kind = match ty {
        "string" => ElicitFieldKind::String {
            format: prop
                .get("format")
                .and_then(|value_f| value_f.as_str())
                .and_then(ElicitTextFormat::from_schema),
            min_length: prop.get("minLength").and_then(|value_v| value_v.as_u64()),
            max_length: prop.get("maxLength").and_then(|value_v| value_v.as_u64()),
            default: default_str,
        },
        "number" => ElicitFieldKind::Number {
            minimum: prop.get("minimum").and_then(|value_v| value_v.as_f64()),
            maximum: prop.get("maximum").and_then(|value_v| value_v.as_f64()),
            default: default_str,
        },
        ...
        "boolean" => ElicitFieldKind::Boolean {
            default: prop
                .get("default")
                .and_then(|value_d| value_d.as_bool())
                .unwrap_or(false),
        },
...
fn multi_select_from_schema(prop: &Value) -> Result<ElicitFieldKind, String> {
    ...
    let options: Vec<ElicitOption> =
        if let Some(values) = items.get("enum").and_then(|value_e| value_e.as_array()) {
            values
                .iter()
                .filter_map(|value_v| {
                    let value = json_scalar_to_string(value_v)?;
            ...
            .and_then(|value_o| value_o.as_array())
        {
    ...
    let default_indexes = prop
        .get("default")
        .and_then(|value_d| value_d.as_array())
        .map(|defaults| {
            defaults
                .iter()
                .filter_map(|value_d| value_d.as_str())
                .filter_map(|value_d| options.iter().position(|value_o| value_o.value == value_d))
                .collect()
        })
        .unwrap_or_default();
    ...
    Ok(ElicitFieldKind::MultiSelect {
        options,
        min_items: prop.get("minItems").and_then(|value_v| value_v.as_u64()),
        max_items: prop.get("maxItems").and_then(|value_v| value_v.as_u64()),
        default_indexes,
    })
}
...
fn const_title_options(entries: &[Value]) -> Vec<ElicitOption> {
    entries
        ...
        .filter_map(|entry| {
            let value = entry
                .get("const")
                .and_then(|value_c| value_c.as_str())?
                .to_string();
            let label = entry
                .get("title")
                .and_then(|value_t| value_t.as_str())
...
fn json_scalar_to_string(value_v: &Value) -> Option<String> {
    match value_v {
        Value::String(value_s) => Some(value_s.clone()),
        Value::Number(value_n) => Some(value_n.to_string()),
        Value::Bool(value_b) => Some(value_b.to_string()),
        _ => None,
    }
}

fn default_option_index(options: &[ElicitOption], default: Option<&str>) -> Option<usize> {
    default.and_then(|value_d| options.iter().position(|value_o| value_o.value == value_d))
}
...
fn integer_bound(value_v: Option<&Value>, lower: bool) -> Option<i64> {
    let value_v = value_v?;
    if let Some(value_i) = value_v.as_i64() {
        return Some(value_i);
    }
    let value_f = value_v.as_f64()?;
    let tightened = if lower {
        value_f.ceil()
    } else {
        value_f.floor()
    };
...
fn check_options(options: &[ElicitOption]) -> Result<(), String> {
    ...
    if options.iter().any(|value_o| {
        !chars_within(&value_o.value, MAX_ELICIT_ENUM_VALUE_CHARS)
            || !chars_within(&value_o.label, MAX_ELICIT_ENUM_VALUE_CHARS)
    }) {
        return Err(format!(
