...
mod tests {
    ...
    fn request_serializes_camel_case() {
        ...
        let value_v = serde_json::to_value(&req).unwrap();
        assert!(value_v.get("sessionId").is_some());
        assert!(value_v.get("toolCallId").is_some());
        assert!(value_v.get("serverName").is_some());
        assert!(value_v.get("requestedSchema").is_some());
        assert!(value_v.get("session_id").is_none());
    }
    ...
    fn response_accept_with_content() {
        ...
        let value_v = serde_json::to_value(&resp).unwrap();
        assert_eq!(value_v["outcome"], "accept");
        assert_eq!(value_v["content"]["email"], "a@b.com");
        let back: McpElicitExtResponse = serde_json::from_value(value_v).unwrap();
