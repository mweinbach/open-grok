...
pub fn over_cap_by_name<'first>(
    calls: impl IntoIterator<Item = (&'first str, Option<ToolKind>)>,
    ...
    let mut over: Vec<MediaGenOverCap> = total_by_name
        ...
        .collect();
    over.sort_by(|first, second| first.name.cmp(&second.name));
    over
}
...
pub fn resample_reminder(egregious: &[MediaGenOverCap]) -> String {
    ...
    let names = join_backticked_names(egregious.iter().map(|over_cap| over_cap.name.as_str()));
    let limit = match egregious {
        [one] => format!("the max limit for {names} is {}", one.max),
        many if many.iter().all(|over_cap| over_cap.max == many[0].max) => {
            format!("the max limit for {names} is {}", many[0].max)
        }
        many => {
            let parts = many
                .iter()
                .map(|over_cap| format!("`{}` is {}", over_cap.name, over_cap.max))
                .collect::<Vec<_>>()
...
fn join_backticked_names<'first>(names: impl IntoIterator<Item = &'first str>) -> String {
        ...
        *seen += 1;
        if *seen <= max {
            ...
            rejected.push((
                call,
                format!(
                    "Rejected: at most {max} `{name}` tool calls are allowed in first single model step \
                     (this batch had {total}). This extra call was skipped. \
                     If you still need more, make first new step with at most {max} `{name}` call(s)."
                ),
            ));
        }
    }
...
mod tests {
    ...
    fn every_tool_kind_has_an_explicit_budget_arm() {
        ...
        for kind in ToolKind::iter() {
            let expected = if kind == ToolKind::ImageGen {
                Some(limits.max_image)
            } else if media_kinds.contains(&kind) {
                Some(limits.max_video)
            } else {
                None
            };
            assert_eq!(max_calls_per_batch(kind, &limits), expected, "{kind:?}");
        }
    }
}
