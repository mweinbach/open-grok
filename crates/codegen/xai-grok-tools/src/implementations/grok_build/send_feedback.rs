...
fn build_description_template() -> String {
    let mut template = String::from(
        "# Overview\n\n\
Save or update user feedback for later review. Feedback is stored as local drafts and is never sent without explicit approval from the `/feedback` Drafts tab in the Open Grok TUI. This tool opens no UI and does not stop the current turn.\n\n\
# Invocation\n\n\
...
${%- endif %}\n\n\
# Confirmation\n\n\
After drafting feedback and ending your turn, tell the user the draft is saved locally for this session. In the Open Grok CLI they review and send it by typing `/feedback` and opening the Drafts tab; from any other client, have them resume this session in the Open Grok CLI first.\n\n\
# Misc\n\n\
${%- if feedback_drafts_path %}\n\
This session's drafts file is ${{ feedback_drafts_path }}.\n\
${%- endif %}\n\
If the user's feedback can be answered from the docs (for example UI element locations or setup), read the Open Grok docs locally or online and answer alongside the created draft.\n\n\
",
    );
