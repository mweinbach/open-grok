    ...
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Accepted { message_id } => {
                write!(formatter, "Message accepted (message_id: {message_id}).")
            }
            Self::NotFoundOrNotOwned => {
                formatter.write_str("Subagent not found or not owned by this session.")
            }
            Self::NotActiveOrFinalizing => formatter.write_str("Subagent is not active or is finalizing."),
            Self::Saturated { max_in_flight } => write!(
                formatter,
                "Message admission is saturated (maximum {max_in_flight} in flight)."
            ),
            Self::AdmissionUncertain => formatter.write_str(
                "Message admission could not be confirmed; the message may or may not have been accepted.",
            ),
            Self::NotAcceptedBeforeDeadline => {
                formatter.write_str("Message was not accepted before the delivery deadline.")
            }
            Self::Unsupported => {
                formatter.write_str("Active agent messages are unsupported in this context.")
            }
            Self::Limit {
                max_bytes,
                observed_bytes,
            ...
            Self::ChannelClosed => {
                formatter.write_str("Message was not accepted because the subagent channel closed.")
            }
        }
    }
}
