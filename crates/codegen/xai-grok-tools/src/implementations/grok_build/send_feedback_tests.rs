...
#[test]
fn description_uses_open_grok_branding() {
    let template = SendFeedbackTool.description_template();
    assert!(template.contains("Open Grok TUI"));
    assert!(template.contains("Open Grok CLI"));
    assert!(template.contains("Open Grok docs"));
    assert!(!template.contains("the Grok TUI"));
    assert!(!template.contains("the Grok CLI"));
    assert!(!template.contains("Grok Build docs"));
}
