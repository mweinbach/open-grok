...
fn rewrite_posix_env_refs_for_powershell<'a>(
    ...
    for reference in crate::env_expand::iter_env_var_references(command) {
        if reference.start < cursor {
            continue;
        }
        if reference.name.is_empty() || reference.has_modifier {
            continue;
        }
        if !RUNNER_ALWAYS_SET_ENV.contains(&reference.name)
            && !extra_env.contains_key(reference.name)
        {
            continue;
        }
        let (quote, escaped) = powershell_ctx_at(command, reference.start);
        if quote == PsQuote::Single || escaped {
            continue;
        }
        let buf = out.get_or_insert_with(|| String::with_capacity(command.len() + 24));
        if quote == PsQuote::Bare {
            let token_end = command[reference.start..]
                .find(|character: char| {
                ...
                .map_or(command.len(), |index| reference.start + index);
            buf.push_str(&command[cursor..reference.start]);
            buf.push('"');
            rewrite_ps_env_refs_in_span(buf, &command[reference.start..token_end], extra_env);
            buf.push('"');
            cursor = token_end;
        } else {
            buf.push_str(&command[cursor..reference.start]);
            push_ps_env_ref(buf, reference.braced, reference.name);
            cursor = reference.end;
        }
        if first_rewrite_at.is_none() {
            first_rewrite_at = Some(reference.start);
        }
    }
    match out {
...
fn rewrite_ps_env_refs_in_span(
    ...
    let mut cur = 0;
    for reference in crate::env_expand::iter_env_var_references(span) {
        if reference.name.is_empty() || reference.has_modifier {
            continue;
        }
        if !RUNNER_ALWAYS_SET_ENV.contains(&reference.name)
        ...
        buf.push_str(&span[cur..reference.start]);
        push_ps_env_ref(buf, reference.braced, reference.name);
        cur = reference.end;
    }
