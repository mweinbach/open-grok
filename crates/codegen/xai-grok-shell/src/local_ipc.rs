//! Shared cross-platform local IPC transport (Unix domain sockets and
//! Windows named pipes), extracted from the leader transport so other
//! machine-local servers (the session bus) can reuse the same path-based
//! API.
//!
//! - **Unix:** [`LocalIpcListener`] / [`LocalIpcStream`] are type aliases for
//!   `tokio::net::UnixListener` / `tokio::net::UnixStream`.
//! - **Windows:** wraps `tokio::net::windows::named_pipe::*` (tokio doesn't
//!   expose AF_UNIX on Windows). The filesystem path is hashed into
//!   `\\.\pipe\<pipe_prefix><hash>` so callers keep their path-based API.
//!
//! Callers pass a `pipe_prefix` (e.g. `grok-leader-`, `grok-sbus-`) that
//! namespaces their pipes on Windows; it is ignored on Unix, where the path
//! is a real filesystem socket path.

use std::io;
use std::path::Path;

#[cfg(unix)]
pub type LocalIpcListener = tokio::net::UnixListener;
#[cfg(unix)]
pub type LocalIpcStream = tokio::net::UnixStream;

/// Bind a listener at `path`. On Unix the parent directory must exist; the
/// caller owns stale-socket cleanup for its own paths (see
/// [`remove_stale_socket`]).
pub fn bind(path: &Path, pipe_prefix: &str) -> io::Result<LocalIpcListener> {
    #[cfg(unix)]
    {
        let _ = pipe_prefix;
    ...
    #[cfg(windows)]
    {
        windows_impl::LocalIpcListener::bind(path, pipe_prefix)
    }
}

/// Connect to a listener bound at `path`.
pub async fn connect(path: &Path, pipe_prefix: &str) -> io::Result<LocalIpcStream> {
    #[cfg(unix)]
    {
        let _ = pipe_prefix;
    ...
    #[cfg(windows)]
    {
        windows_impl::LocalIpcStream::connect(path, pipe_prefix).await
    }
}

/// Has a listener been bound at `path`?
///
...
pub fn listener_is_ready(path: &Path, pipe_prefix: &str) -> bool {
    #[cfg(unix)]
    {
        let _ = pipe_prefix;
        path.exists()
    ...
    #[cfg(windows)]
    {
        windows_impl::listener_is_ready(path, pipe_prefix)
    }
}

/// Best-effort removal of a stale Unix socket file (Windows named pipes
/// vanish with their owning process, so this is a no-op there).
...
/// Restrict a Unix domain socket to the owning user (0600). Best-effort: the
/// enclosing `$OPENGROK_HOME` is normally 0700 already; this is defense in
/// depth. No-op on Windows (named-pipe ACLs default to the creating user).
pub fn restrict_socket_permissions(path: &Path) {
    #[cfg(unix)]
    {
...
#[cfg(windows)]
pub use windows_impl::{LocalIpcListener, LocalIpcStream};
...
mod windows_impl {
    ...
    impl LocalIpcStream {
        ...
        /// named-pipe name (namespaced by `pipe_prefix`) and
        /// `ClientOptions::open` is used.
        pub async fn connect(path: &Path, pipe_prefix: &str) -> io::Result<Self> {
            ...
            let pipe_name = path_to_pipe_name(path, pipe_prefix);
    ...
    impl LocalIpcListener {
        ...
        pub fn bind(path: &Path, pipe_prefix: &str) -> io::Result<Self> {
        ...
        pub async fn accept(&self) -> io::Result<(LocalIpcStream, ())> {
            use tokio::net::windows::named_pipe::ServerOptions;
            ...
            Err(last_err
                .unwrap_or_else(|| io::Error::other("LocalIpcListener: accept exhausted retries")))
        }
    }

    /// Whether a pipe has been bound at `path`.
    ///
    /// Probes with `WaitNamedPipeW` (non-connecting), not `ClientOptions::open`,
    /// which would open a real client the listener's `accept()` consumes as a
    ...
    /// Full named-pipe path: `\\.\pipe\<prefix><hash>`.
    fn path_to_pipe_name(path: &Path, pipe_prefix: &str) -> std::ffi::OsString {
        let mut name = std::ffi::OsString::from(r"\\.\pipe\");
        name.push(pipe_leaf_name(path, pipe_prefix));
        name
    }

    /// Deterministic leaf name (`<prefix><hash>`) for a filesystem path.
    ///
    ...
    fn pipe_leaf_name(path: &Path, pipe_prefix: &str) -> std::ffi::OsString {
        ...
        std::ffi::OsString::from(format!("{pipe_prefix}{hash:016x}"))
    }
    ...
    mod tests {
        ...
        fn pipe_name_is_deterministic() {
            let a = path_to_pipe_name(Path::new("/tmp/grok.sock"), "grok-leader-");
            let b = path_to_pipe_name(Path::new("/tmp/grok.sock"), "grok-leader-");
            assert_eq!(a, b);
        }

        #[test]
        fn different_paths_produce_different_names() {
            let a = path_to_pipe_name(Path::new("/tmp/a.sock"), "grok-leader-");
            let b = path_to_pipe_name(Path::new("/tmp/b.sock"), "grok-leader-");
            assert_ne!(a, b);
        }

        #[test]
        fn prefixes_produce_distinct_namespaces() {
            let a = path_to_pipe_name(Path::new("/tmp/a.sock"), "grok-leader-");
            let b = path_to_pipe_name(Path::new("/tmp/a.sock"), "grok-sbus-");
            assert_ne!(a, b);
        }

        #[test]
        fn pipe_name_has_correct_prefix() {
            let name = path_to_pipe_name(Path::new("/tmp/test.sock"), "grok-leader-");
        ...
        fn pipe_name_is_bounded() {
            ...
            let name = path_to_pipe_name(Path::new(&long_path), "grok-leader-");
        ...
        async fn listener_is_ready_tracks_pipe_lifecycle() {
            ...
            assert!(!listener_is_ready(&path, "grok-leader-"));

            let listener = LocalIpcListener::bind(&path, "grok-leader-").unwrap();
