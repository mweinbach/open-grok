...
const SETTINGS_CACHE_HMAC_KEY: &[u8] =
    b"open-grok-shell-settings-cache-hmac-v1-ba6c43d3-404f-4b5c-b0cd-df09b2f5bdf4";
...
impl SettingsCacheManager {
    ...
    pub(crate) fn load_or_fetch(
        ...
        if crate::agent::config::env_bool("OPENGROK_SETTINGS_CACHE")
            .or_else(|| crate::agent::config::env_bool("GROK_SETTINGS_CACHE"))
            == Some(false)
        {
            return (fetch(), None);
        }
    ...
    fn load_fresh(&self, identity: &str, origin: &str) -> Option<CachedSettings> {
        ...
        let signed: SignedSettingsCache = serde_json::from_slice(&data)
            .inspect_err(|error| tracing::debug!(error = %error, "settings cache parse failed"))
            .ok()?;
...
fn sign_cache_payload(payload: &[u8]) -> Vec<u8> {
    use hmac::Mac;
    let Ok(mut mac) = HmacSha256::new_from_slice(SETTINGS_CACHE_HMAC_KEY) else {
        return Vec::new();
    };
    mac.update(payload);
...
mod settings_cache_tests {
    ...
    fn hand_written_file_with_stale_version_or_client_misses() {
        ...
        let write =
            |cache: &SettingsCache| std::fs::write(&path, signed_cache_bytes(cache)).unwrap();
