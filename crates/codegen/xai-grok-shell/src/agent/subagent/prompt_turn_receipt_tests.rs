        ...
        PromptTurnReceipt {
            ...
            telemetry: Some(
                crate::session::telemetry::ActiveAgentMessageTelemetrySource::new(
                    "parent-session".into(),
                    Arc::new(std::sync::atomic::AtomicUsize::new(turn)),
...
async fn receipts_settle_concurrently_while_preserving_fifo_authority() {
    ...
    assert_eq!(
        telemetry
            .expect("latest message telemetry")
            .parent_prompt_index(),
        4
    );
    assert_eq!(
...
fn dropped_oneshot_error() -> oneshot::error::RecvError {
    let (sender, receiver) = oneshot::channel::<crate::session::commands::PromptTurnResult>();
    drop(sender);
    futures::FutureExt::now_or_never(receiver)
...
async fn uncertain_settlement_deadline_forces_turn_shutdown() {
    ...
    assert!(matches!(
        ...
        Some(FinalPromptTurnReceipt {
            ...
            && telemetry.as_ref().is_some_and(|telemetry| telemetry.parent_prompt_index() == 4)
    ));
}
