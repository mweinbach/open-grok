...
pub(crate) struct PromptTurnReceipt {
    ...
    pub telemetry: Option<crate::session::telemetry::ActiveAgentMessageAdmissionTelemetry>,
}
        ...
        tokio::select! {
            ...
            receipt = receipt_stream.recv(), if is_stream_open => {
                match receipt {
                    Some(receipt) => {
                        ...
                        final_telemetry = receipt.telemetry.clone();
            ...
            _ = async {
                ...
                while let Ok(receipt) = receipt_stream.try_recv() {
                    ...
                    final_telemetry = receipt.telemetry;
                }
                while pending.next().now_or_never().flatten().is_some() {}
                return force_receipt_settlement_timeout(child_cmd_tx, final_identity, final_telemetry);
            }
        }
    }
}
...
fn force_receipt_settlement_timeout(
    ...
    final_telemetry: Option<crate::session::telemetry::ActiveAgentMessageAdmissionTelemetry>,
) -> PromptTurnReceiptSettlement {
