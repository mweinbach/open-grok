...
/// `$OPENGROK_HOME/skills/`; `help` rows hash the pre-substitution bytes.
...
/// Remove platform-skill leftovers extracted into `$OPENGROK_HOME/skills/` by
...
fn purge_skill_dirs_matching(grok_home: &std::path::Path, known: &[(&str, &str)]) {
    // For reversing the extract-time `~/.opengrok/` -> home rewrite (help only).
    ...
    for name in names {
        ...
        let unrewritten = sha256_hex(content.replace(&home_prefix, "~/.opengrok/").as_bytes());
        let managed = known
            .iter()
...
mod tests {
    ...
    fn purge_reverses_home_rewrite_before_hashing() {
        ...
        let raw =
            EXTRACTED_BODY.replace("platform", "read ~/.opengrok/docs/user-guide then platform");
        let body = raw.replace("~/.opengrok/", &format!("{}/", home.to_string_lossy()));
