...
pub(crate) async fn new(
    ...
    let (handle, rx, summary_tx, disk_full_tx) = actor_channel(provider_boundary.clone());
...
mod actor_lifetime_tests {
    ...
    async fn dropping_the_session_handle_closes_the_actor_channel() {
        let (handle, mut rx, summary_tx, _disk_full_tx) =
            actor_channel(ProviderBoundary::default());
