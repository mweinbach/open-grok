...
pub(crate) const BUILTIN_WORKFLOWS: &[BuiltinWorkflow] = &[
    ...
    BuiltinWorkflow {
        name: "ultracode",
        script: include_str!("../workflows/ultracode.rhai"),
        path: concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/src/session/workflows/ultracode.rhai"
        ),
    },
];
...
impl WorkflowRegistry {
    pub(crate) fn scan(session_cwd: Option<&Path>) -> Self {
        ...
        {
            dirs.push((
                project_root(cwd).join(".opengrok").join("workflows"),
                "project",
            ));
        }
...
fn bundled_file_is_managed(path: &Path) -> bool {
    ...
    let relative = relative
        ...
        .collect::<Vec<_>>()
        .join("/");
    xai_grok_bundle::is_managed_bundle_file(root, &relative)
}
...
pub(crate) fn save_project_workflow(
    ...
    let dir = canonical_root.join(".opengrok").join("workflows");
...
mod tests {
    ...
    fn deterministic_scan_uses_git_root_and_skips_invalid_filename() {
        ...
        let wf_dir = dir.path().join(".opengrok").join("workflows");
    ...
    fn project_workflows_follow_folder_trust() {
        let dir = tempfile::tempdir().unwrap();
        git2::Repository::init(dir.path()).unwrap();
        let workflows = dir.path().join(".opengrok/workflows");
    ...
    fn managed_bundled_override_keeps_builtin_privileges() {
        ...
        let checksum = xai_grok_bundle::checksum_file(&path).unwrap();
    ...
    fn discovery_and_explicit_paths_reject_symlink_files() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let project = dir.path().join("project");
        let workflows = project.join(".opengrok/workflows");
    ...
    fn save_through_symlinked_session_root_stays_in_canonical_project() {
        ...
        assert_eq!(
            ...
            dunce::canonicalize(&project)
                .unwrap()
                .join(".opengrok/workflows/safe.rhai")
        );
    }
    ...
    fn save_rejects_symlinked_registry_directory() {
        ...
        std::fs::create_dir_all(project.join(".opengrok")).unwrap();
        std::fs::create_dir_all(&attacker).unwrap();
        symlink(&attacker, project.join(".opengrok/workflows")).unwrap();
