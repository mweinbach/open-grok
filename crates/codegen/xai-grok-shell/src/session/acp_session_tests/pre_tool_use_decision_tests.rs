...
async fn pre_tool_use_updated_input_rewrites_prepared_call() {
    ...
    local
        .run_until(async {
            ...
            let actor = Arc::new(
                actor_with_pre_tool_use_hook(REWRITE_TARGET_FILE_HOOK, true, gateway_tx).await,
            );
...
async fn pre_tool_use_ask_hook_forces_prompt_under_yolo_and_runs_on_approve() {
    ...
    local
        .run_until(async {
            let (gateway_tx, gateway_rx) =
                tokio::sync::mpsc::unbounded_channel::<xai_acp_lib::AcpClientMessage>();
            let actor = Arc::new(actor_with_pre_tool_use_hook(ASK_HOOK, true, gateway_tx).await);
            let gateway = RecordingGateway::spawn(gateway_rx, PromptAnswer::Select("allow-once"));
...
async fn pre_tool_use_ask_hook_does_not_double_prompt_in_default_mode() {
    ...
    local
        .run_until(async {
            let (gateway_tx, gateway_rx) =
                tokio::sync::mpsc::unbounded_channel::<xai_acp_lib::AcpClientMessage>();
            let actor = Arc::new(actor_with_pre_tool_use_hook(ASK_HOOK, false, gateway_tx).await);
            let gateway = RecordingGateway::spawn(gateway_rx, PromptAnswer::Select("allow-once"));
...
async fn pre_tool_use_allow_does_not_auto_approve() {
    ...
    local
        .run_until(async {
            let (gateway_tx, gateway_rx) =
                tokio::sync::mpsc::unbounded_channel::<xai_acp_lib::AcpClientMessage>();
            let actor = Arc::new(actor_with_pre_tool_use_hook(ALLOW_HOOK, false, gateway_tx).await);
            let gateway = RecordingGateway::spawn(gateway_rx, PromptAnswer::Select("allow-once"));
