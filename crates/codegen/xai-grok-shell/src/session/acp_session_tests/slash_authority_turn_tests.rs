...
struct DynamicResolutionRecorder {
    events: tokio::sync::mpsc::UnboundedReceiver<SessionEvent>,
    command_advertisements: usize,
}

impl DynamicResolutionRecorder {
    fn calls(&mut self) -> crate::session::slash_authority::DynamicResolutionCalls {
        while let Ok(event) = self.events.try_recv() {
            ...
            {
                self.command_advertisements += 1;
            }
        }
        let calls = crate::session::slash_authority::dynamic_resolution_calls();
        crate::session::slash_authority::DynamicResolutionCalls {
            skill_catalog: calls
                .skill_catalog
                .checked_sub(self.command_advertisements)
                .expect("each command advertisement snapshots the skill catalog"),
            workflow_discovery: calls
                .workflow_discovery
                .checked_sub(self.command_advertisements)
                .expect("each command advertisement snapshots named workflows"),
            ..calls
        }
    }
}
...
#[test]
fn resolution_recorder_excludes_only_observed_command_advertisements() {
    let (event_tx, events) = tokio::sync::mpsc::unbounded_channel();
    let mut recorder = DynamicResolutionRecorder {
        events,
        command_advertisements: 0,
    };
    ...
    let advertisement = acp::SessionNotification::new(
        acp::SessionId::new("authority-recorder"),
        acp::SessionUpdate::AvailableCommandsUpdate(acp::AvailableCommandsUpdate::new(Vec::new())),
    );
    assert!(
        event_tx
            .send(SessionEvent::Notification(advertisement.into()))
            .is_ok()
    );
    ...
    crate::session::slash_authority::record_workflow_discovery_call();
    let after = recorder.calls();
    assert_eq!(after.skill_catalog, before.skill_catalog + 1);
    assert_eq!(after.command_availability, before.command_availability + 1);
    assert_eq!(after.workflow_discovery, before.workflow_discovery + 1);
}
...
fn human_non_slash_runs_dynamic_preparation_but_model_non_slash_does_not() {
    run_on_session_sized_stack(|| {
        Box::pin(async {
            ...
            let (actor, _hook_rx, _user_chunk_rx, _policy_recorder, mut resolution_recorder) =
                actor_with_sampler(
                    &server,
            ...
            let before = resolution_recorder.calls();
            ...
            let after_human = resolution_recorder.calls();
            ...
            let after_model = resolution_recorder.calls();
            ...
            assert_eq!(
                after_model.workflow_discovery,
                after_human.workflow_discovery
            );
        })
    });
}
...
fn runtime_control_slash_stays_inert_without_dynamic_catalogs() {
    run_on_session_sized_stack(|| {
        Box::pin(async {
            ...
            let (actor, mut hook_rx, _user_chunk_rx, policy_recorder, mut resolution_recorder) =
                actor_with_sampler(
                    &server,
                    Arc::new(RecordingTerminal {
                        calls: Arc::new(AtomicUsize::new(0)),
...
fn parent_compact_and_available_skill_execute_but_other_slashes_stay_inert() {
    run_on_session_sized_stack(|| {
        Box::pin(async {
            ...
            let (actor, mut hook_rx, mut user_chunk_rx, policy_recorder, mut resolution_recorder) =
                actor_with_sampler(
                    &server,
                    Arc::new(RecordingTerminal {
                        calls: Arc::new(AtomicUsize::new(0)),
            ...
            actor.goal_plan_reconciled.store(false, Ordering::Relaxed);
            let dynamic_calls_before_compact = resolution_recorder.calls();
            let compact_result =
            ...
            assert!(!actor.goal_plan_reconciled.load(Ordering::Relaxed));
            let dynamic_calls_after_compact = resolution_recorder.calls();
            assert_eq!(
            ...
            let calls_before_unavailable = resolution_recorder.calls();
            run_parent_turn(
                &actor,
            ...
            let calls_after_unavailable = resolution_recorder.calls();
            assert_eq!(
            ...
            assert_eq!(
                calls_after_unavailable.workflow_discovery,
                calls_before_unavailable.workflow_discovery
            );
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
            ...
            let calls_before_skill = resolution_recorder.calls();
            run_parent_turn(
                &actor,
            ...
            let calls_after_skill = resolution_recorder.calls();
            assert_eq!(
            ...
            assert_eq!(
                calls_after_skill.workflow_discovery,
                calls_before_skill.workflow_discovery
            );
            ...
            assert!(hook_rx.try_recv().is_err());
            let calls_after_inert_slashes = resolution_recorder.calls();
            assert_eq!(
            ...
            assert_eq!(
                calls_after_inert_slashes.workflow_discovery,
                calls_after_skill.workflow_discovery
            );
...
fn parent_skill_lookup_matches_advertised_gated_collision_and_skill_only_loader() {
    run_on_session_sized_stack(|| {
        Box::pin(async {
            ...
            let (actor, _hook_rx, _user_chunk_rx, _policy_recorder, _resolution_recorder) =
                actor_with_sampler(
                    &server,
                    Arc::new(RecordingTerminal {
                        calls: Arc::new(AtomicUsize::new(0)),
...
fn parent_bash_metadata_and_placeholder_path_cannot_reach_host_routes() {
    run_on_session_sized_stack(|| {
        Box::pin(async {
            ...
            let (actor, mut hook_rx, _user_chunk_rx, policy_recorder, _resolution_recorder) =
                actor_with_sampler(
                    &server,
                    Arc::new(RecordingTerminal {
