...
#[test]
fn blocked_user_prompt_cancels_turn_without_sampling() {
...
#[test]
fn hook_system_message_reaches_user_as_annotation() {
    run_on_session_sized_stack(|| {
        Box::pin(async {
            let (gateway_tx, _gateway_rx) =
                mpsc::unbounded_channel::<xai_acp_lib::AcpClientMessage>();
...
#[test]
fn hold_suppresses_idle_injection() {
    run_on_session_sized_stack(|| {
        Box::pin(async {
            let (gateway_tx, _gateway_rx) =
                mpsc::unbounded_channel::<xai_acp_lib::AcpClientMessage>();
...
#[test]
fn flushed_interjection_stays_parked_under_hold() {
    run_on_session_sized_stack(|| {
        Box::pin(async {
            let (gateway_tx, _gateway_rx) =
                mpsc::unbounded_channel::<xai_acp_lib::AcpClientMessage>();
...
#[test]
fn noop_queue_mutations_report_unchanged() {
    run_on_session_sized_stack(|| {
        Box::pin(async {
            let (gateway_tx, _gateway_rx) =
                mpsc::unbounded_channel::<xai_acp_lib::AcpClientMessage>();
            ...
            let send_now = actor
                ...
                .await;
            assert!(!send_now, "a stale send-now must report unchanged");
        })
    });
}
...
#[test]
fn hook_denied_completion_holds_queue_until_release() {
    run_on_session_sized_stack(|| {
        Box::pin(async {
            let (gateway_tx, _gateway_rx) =
                mpsc::unbounded_channel::<xai_acp_lib::AcpClientMessage>();
            ...
            *actor
            ...
            let (epoch, task_identity) = completion_binding(&actor, "p-hook").await;
            actor
                .handle_completion(
                    "p-hook".to_string(),
                    epoch,
...
#[test]
fn hook_denied_completion_does_not_rearm_cleared_hold() {
    run_on_session_sized_stack(|| {
        Box::pin(async {
            let (gateway_tx, _gateway_rx) =
                mpsc::unbounded_channel::<xai_acp_lib::AcpClientMessage>();
...
#[test]
fn non_hook_cancel_does_not_hold_queue() {
    run_on_session_sized_stack(|| {
        Box::pin(async {
            let (gateway_tx, _gateway_rx) =
                mpsc::unbounded_channel::<xai_acp_lib::AcpClientMessage>();
            ...
            *actor
            ...
            let (epoch, task_identity) = completion_binding(&actor, "p-user").await;
            actor
                .handle_completion(
                    "p-user".to_string(),
                    epoch,
...
#[test]
fn blocked_prompt_resolves_persist_ack() {
    run_on_session_sized_stack(|| {
        Box::pin(async {
            let (gateway_tx, _gateway_rx) =
                mpsc::unbounded_channel::<xai_acp_lib::AcpClientMessage>();
...
#[test]
fn blocked_turn_preserves_prior_interrupt_marker() {
    run_on_session_sized_stack(|| {
        Box::pin(async {
            let (gateway_tx, _gateway_rx) =
                mpsc::unbounded_channel::<xai_acp_lib::AcpClientMessage>();
...
#[test]
fn blocked_turn_preserves_redirect_marker() {
    run_on_session_sized_stack(|| {
        Box::pin(async {
            let (gateway_tx, _gateway_rx) =
                mpsc::unbounded_channel::<xai_acp_lib::AcpClientMessage>();
