...
pub trait StorageAdapter: Send + Sync {
    ...
    async fn regenerate_generated_title(&self, info: &Info, title: String) -> io::Result<bool>;
    ...
    async fn sync_session_files(&self, info: &Info) -> io::Result<()> {
        self.sync_session_files_selected(info, SessionFileSet::ALL)
            .await
    }
...
pub(crate) fn prepare_replay_lines_with_transport_ids<'a>(
    ...
    let cursor_pos = cursor
        ...
        .filter(|&pos| {
            let bounded = filtered[pos + 1..].iter().all(|l| {
                replay::line_is_dropped_on_replay(l)
    ...
    for (i, &line) in filtered.iter().enumerate() {
        if replay::line_is_dropped_on_replay(line)
    ...
    PreparedReplay {
        ...
        max_event_seq,
        total_live,
        unfinished_subagents: replay::collect_unfinished_subagents(&filtered),
    }
}
...
pub(crate) fn filter_delta_replay_lines_with_prior_and_transport_ids<'a>(
    ...
    let live: Vec<&str> = contents
        .lines()
        .filter(|l| !l.trim().is_empty() && !replay::line_is_dropped_on_replay(l))
        .collect();
...
mod tests {
    ...
    // ── Open Grok Code Mode transport replay ─────────────────────────────
