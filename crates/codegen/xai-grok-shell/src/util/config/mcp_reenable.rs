...
mod tests {
    ...
    /// Git repo with project `.opengrok/config.toml` so discovery is bounded to cwd.
    fn project_repo(toml: &str) -> tempfile::TempDir {
        ...
        std::fs::write(tmp.path().join(".opengrok").join("config.toml"), toml).unwrap();
        tmp
    }

    /// Isolate HOME/OPENGROK_HOME so ambient user MCP config cannot pad discovery.
    fn isolated_home() -> (
        tempfile::TempDir,
        ...
        let grok_guard = xai_grok_test_support::EnvGuard::set("OPENGROK_HOME", &grok_home);
...
"#,
    ...
    #[test]
    #[serial_test::serial]
    fn toml_same_url_named_servers_remain_distinct_in_merge() {
        let (_home, _hg, _gg) = isolated_home();
        let repo = project_repo(
            r#"
[mcp_servers.first]
