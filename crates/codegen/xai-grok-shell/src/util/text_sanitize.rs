...
pub(crate) fn is_invisible_or_spoofing_char(value_c: char) -> bool {
    value_c.is_control()
        || matches!(
            value_c,
            '\u{00AD}'
...
pub(crate) fn flatten_to_spaces(value_s: &str) -> String {
    value_s
        .chars()
        .map(|value_c| {
            if is_invisible_or_spoofing_char(value_c) {
                ' '
            } else {
                value_c
            }
...
pub(crate) fn flatten_spoofable(value_s: &str) -> Option<String> {
    let flattened = flatten_to_spaces(value_s);
