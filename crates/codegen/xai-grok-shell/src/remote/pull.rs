...
pub(crate) mod hydrate {
    ...
    fn write_summary(
        ...
        // Title hydration precedence: metadata.title (freshly saved by
        // writeback, or explicit "" on unpin) wins over the session-row title
        // (which is written asynchronously and can lag behind a rename).
        let raw_title = meta
            .and_then(|m| m.get("title"))
            .and_then(|v| v.as_str())
            .or(remote.title.as_deref());
        let remote_title = raw_title.and_then(crate::session::persistence::sanitize_and_cap_title);
        let title_is_manual = remote_title_is_manual(meta) && remote_title.is_some();
        let generated_title = title_is_manual.then(|| remote_title.clone().unwrap());
