...
async fn provider_expiry_source_precedence() {
    ...
    fn jwt_with_exp(exp: i64) -> String {
        // The fork's workspace unifies jsonwebtoken's `rust_crypto` and
        // `aws_lc_rs` crypto features, so the provider can't be auto-selected
        // from crate features; install it explicitly before encoding (mirrors
        // `manager_tests::ensure_crypto_provider`).
