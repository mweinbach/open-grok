...
fn spawn_lock_holder_subprocess(
...

    {
        ...
        loop {
            line.clear();
            let bytes_read = reader.read_line(&mut line).expect("read child stdout");
            assert!(bytes_read > 0, "child exited before signaling ready");
...
async fn dead_recorded_pid_with_live_flock_is_never_broken() {
    ...
    let mut child = spawn_lock_holder_subprocess(&lock_path, "dead_pid", 0);
    let inode_before = inode_of(&lock_path);
    ...
    assert_eq!(
        holder.map(|holder| (holder.state, holder.pid)),
