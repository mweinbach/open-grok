    ...
    fn spawn(mut file: File, interval: StdDuration) -> Self {
        ...
        let handle = std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                {
                    if let Err(error) = write_holder_info(&mut file) {
                        tracing::debug!(error = %error, "auth lock: heartbeat rewrite failed");
                    }
                }
            })
            .inspect_err(|error| {
                unified_log::warn(
                    &format!("auth lock: failed to spawn heartbeat thread: {error}"),
                    None,
                    None,
                );
            })
...
fn is_process_alive(pid: u32) -> bool {
    let pid_i = match i32::try_from(pid) {
        Ok(process) if process > 0 => process,
...
fn try_acquire_once(lock_path: &Path) -> LockAttempt {
        ...
        .open(lock_path)
    {
        ...
        Err(error) => {
            unified_log::warn(
                &format!("auth lock: failed to open {}: {error}", lock_path.display()),
                None,
                None,
            );
    ...
    match file.try_lock_exclusive() {
        Ok(()) => {
            ...
            if let Err(error) = write_holder_info(&mut file) {
                unified_log::warn(
                    &format!("auth lock: failed to write holder info: {error}"),
                    None,
            ...
            match inodes_match(&file, lock_path) {
                ...
                Err(error) => {
                    unified_log::debug(
                        &format!("auth lock: path gone after acquire (pid={pid}): {error}"),
                        None,
                        None,
                    );
                    LockAttempt::InodeChanged
        ...
        Err(error) if error.kind() == io::ErrorKind::WouldBlock => LockAttempt::Busy,

        Err(error) => {
            unified_log::warn(&format!("auth lock: flock failed: {error}"), None, None);
...
fn blocking_acquire(lock_path: &Path) -> io::Result<File> {
    ...
    loop {
        match file.lock_exclusive() {
            ...
            Err(error) => {
                unified_log::warn(
                    &format!("auth lock: blocking flock failed: {error}"),
                    None,
                    None,
                );
                return Err(error);
    ...
    match inodes_match(&file, lock_path) {
        ...
        Err(error) => Err(io::Error::other(format!(
            "path gone after blocking flock: {error}"
        ))),
    }
}
...
pub(crate) fn try_lock_auth_file_nonblocking(auth_json_path: &Path) -> Option<AuthFileLock> {
    ...
    if let Err(error) = write_holder_info(&mut file) {
        unified_log::warn(
            &format!("auth lock: failed to write holder info (non-blocking): {error}"),
            None,
...
fn lock_guard(file: File, heartbeat: Heartbeat) -> AuthFileLock {
    let heartbeat = match heartbeat {
        ...
        Heartbeat::Attach => match file.try_clone() {
            ...
            Err(error) => {
                unified_log::warn(
                    &format!("auth lock: failed to clone FD for heartbeat: {error}"),
                    None,
                    None,
                );
                None
...
pub(crate) async fn try_lock_auth_file_async(
    ...
    let late_ticket = loop {
        ...
        match tokio::time::timeout(remaining, ticket.claim()).await {
            ...
            Ok(Some(Err(error))) => {
                tracing::debug!(error = %error, "auth lock: shared wait deposited an error");
                tokio::time::sleep(
    ...
    unified_log::warn(
        ...
        Some(serde_json::json!({
            ...
            "holder_state": holder.map(|holder| holder.state.label()),
            "holder_age_secs": holder.and_then(|holder| holder.age_secs),
        })),
    );
...
fn salvage_at_deadline(
    ...
    if let Some(ticket) = ticket {
        match ticket.try_claim() {
            ...
            Some(Err(error)) => deposit_error = Some(error),
            None => {}
        }
    }
    match try_acquire_once(lock_path) {
