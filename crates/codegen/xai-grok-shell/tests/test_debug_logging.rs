...
//! caller-owned `$OPENGROK_HOME`, then inspects `~/.opengrok/debug/`:
...
//!   `~/.opengrok/debug/<sessionId>.txt` with first-party content, and does NOT enable
...
//!   `~/.opengrok/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.opengrok/debug/` files).
...
/// The per-session firehose directory under a pinned `$OPENGROK_HOME`.
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".opengrok").join("debug")
}

/// List firehose `*.txt` files under `~/.opengrok/debug` (excluding the `latest.txt`
...
/// Build a headless `open-grok -p` command with a pinned `$OPENGROK_HOME` so the firehose
/// lands under `<home>/.opengrok/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    cmd.env("OPENGROK_HOME", home.join(".opengrok"));
...
/// headless `open-grok -p` client is near-silent, so its lazily-opened firehose may
...
/// A live `agent` session writes `~/.opengrok/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        let client = GrokStdioClient::spawn_with_home_and_env(
            ...
            workdir.path(),
            home,
            &[("GROK_DEBUG_LOG", "1"), ("OPENGROK_HOME", &grok_home_str)],
        )
        .await;
        ...
        // file, so `tail -f ~/.opengrok/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // Drive `open-grok --debug agent stdio`: the master switch (which runs before
        ...
        // `~/.opengrok/logs/sampling.jsonl` is never written — the `--debug`
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.opengrok/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
