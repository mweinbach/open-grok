...
use crate::stream::{stream_chat_completions, stream_google_ai_studio, stream_messages};
...
async fn run_one_attempt(
    ...
    match client.api_backend() {
        ...
        ApiBackend::GoogleAiStudio => {
            let (raw, metadata) = match client.conversation_stream_google_ai_studio(request).await {
                Ok(pair) => pair,
                Err(e) => return AttemptOutcome::InitFailed { error: e },
            };
            let (teed, captured) = tee_errors(raw);
            let l2 = stream_google_ai_studio(teed, metadata, request_id.clone(), idle_timeout);
            drive_l2(
                l2,
                request_id,
                event_tx,
