...
//! write-deny: `$OPENGROK_HOME/hooks`, `hooks-paths`, and absolute registry targets.
...
pub enum GlobalHookSourceKind {
    ...
    HookDirectory,
    /// `$OPENGROK_HOME/hooks-paths` (protected; never loaded as hook JSON).
    RegistryFile,
...
pub enum GlobalHookSourceError {
    ...
    #[error("symlinked OPENGROK_HOME is not allowed under sandbox write-deny: {path}")]
