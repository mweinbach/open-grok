...
/// allocating until the process dies. Session startup and the file watcher
/// both use this path.
...
/// The hash stored in HEAD, without loading the referenced commit object.
...
/// HEAD's commit hash for cache validation and `--restore-code`.
/// Returns `None` outside a git repository or before its first commit.
