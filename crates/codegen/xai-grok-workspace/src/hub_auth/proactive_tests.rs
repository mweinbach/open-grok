...
#[allow(clippy::await_holding_lock)]
#[tokio::test(flavor = "current_thread")]
async fn endpoint_errors_do_not_log_response_bodies() {
    let _metrics = lock_metrics();
    let issuer = spawn_mock_idp(
        serde_json::json!({"error_description": "echoed-refresh-token"}),
        axum::http::StatusCode::BAD_REQUEST,
        Arc::new(AtomicU32::new(0)),
    )
    .await;
    let mut parameters = provider_params(issuer, None);
    parameters.refresh.enabled = false;
    let provider = ProactiveOidcAuthProvider::new(parameters);
    let error = do_refresh(&provider.inner, "echoed-refresh-token")
        .await
        .err()
        .expect("endpoint rejects refresh");
    assert!(error.terminal);
    assert!(error.to_string().contains("400"));
    assert!(!format!("{error:?}").contains("echoed-refresh-token"));
}
