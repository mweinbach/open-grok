...
/// `.claude` / `.opengrok/agents` / subdir-launch gaps. NOT itself the trust gate.
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    // Project `.opengrok/config.toml` declaring repo-controlled code-exec: a
    ...
    if hook_root.join(".opengrok").join("hooks").is_dir()
