...
#[test]
fn inbound_preserves_fork_edit_content_bytes() {
    let virtualized = mapping();
    let arguments = json!({
        "filePath": "/workspace/file.txt",
        "content": "/workspace/old.txt",
        "oldString": "/workspace/old.txt",
        "newString": "/workspace/new.txt",
        "edits": [{"op": "write", "content": "/workspace/literal.txt"}]
    });
    let rewritten = virtualized.rewrite_json_inbound(arguments.clone());
    assert_eq!(rewritten["filePath"], "/workspace/conv-abc/file.txt");
    for key in ["content", "oldString", "newString", "edits"] {
        assert_eq!(rewritten[key], arguments[key]);
    }
}
...
fn typed_output_and_error_rewrite_model_facing_fields() {
    ...
    let cco = xai_tool_runtime::ToolChatCompletionResponse {
        ...
        stream_error: Some(xai_tool_runtime::ToolStreamError {
