...
mod tests {
    ...
    async fn renamed_active_message_handler_keeps_semantic_hitl_classification() {
        let workspace = crate::handle::tests::make_handle();
        let mut config = ToolConfig::for_tool::<
            xai_grok_tools::implementations::grok_build::SendSubagentMessageTool,
        >();
        config.name_override = Some("relay_to_subagent".to_owned());
        assert_eq!(config.kind, Some(ToolKind::ActiveAgentMessage));
        let name = config.name_override.expect("renamed tool");
        let handler = SessionRoutedToolHandler::new(
            name.clone(),
            ToolDescription::new(name, "relay"),
            config.kind,
            None,
            workspace,
        )
        ...
        let access = handler
            .permission_access(&serde_json::json!({
                "subagent_id": "sub-1",
                "text": "private follow-up",
            }))
            .expect("semantic agent messages must remain guarded");
        assert!(matches!(
            &access,
            crate::permission::AccessKind::AgentMessage { subagent_id } if subagent_id == "sub-1"
        ));
    ...
    #[tokio::test]
    async fn description_kind_cannot_supply_permission_semantics() {
        let workspace = crate::handle::tests::make_handle();
        let mut description = ToolDescription::new("custom_tool", "custom");
        description.kind = Some("active_agent_message".to_owned());
        let handler = SessionRoutedToolHandler::new(
        ...
        assert!(
            handler
                .permission_access(&serde_json::json!({"subagent_id": "sub-1"}))
                .is_none()
        );
    }
