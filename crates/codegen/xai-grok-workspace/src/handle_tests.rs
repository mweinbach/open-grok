...
#[tokio::test]
async fn routed_handler_kind_comes_from_registry_after_rename() {
    let handle = make_handle();
    let mut renamed = tc("GrokBuild:read_file", None);
    renamed.name_override = Some("inspect_source".to_owned());
    let session = handle
        ...
        .expect("renamed registry-backed session");
    let handlers = build_session_routed_handlers(&session.toolset(), &handle);
    let handler = handlers
        .iter()
        .find(|handler| handler.tool_id().as_str() == "inspect_source")
        .expect("renamed handler");
    assert_eq!(handler.description().kind.as_deref(), Some("read"));
}
...
#[tokio::test]
async fn rebind_cannot_change_session_root_or_relocate_files() {
    let directory = tempfile::tempdir().unwrap();
    let first_root = directory.path().join("first");
    let rejected_root = directory.path().join("rejected");
    std::fs::create_dir_all(&first_root).unwrap();
    std::fs::write(first_root.join("private.txt"), "keep in original session").unwrap();
    let handle = make_handle();
    let resolver = bind_resolver_fixture(&handle);
    let session_id = xai_tool_protocol::SessionId::new("immutable-root").unwrap();
    resolver(
        session_id.clone(),
        Some(serde_json::json!({
            "cwd": "/workspace",
            "metadata": {"session_root": first_root},
        })),
    )
    .await
    .expect("initial bind");
    let error = resolver(
        ...
        Some(serde_json::json!({
            "cwd": "/workspace",
            "metadata": {"session_root": rejected_root},
        })),
    )
    .await
    .err()
    .expect("divergent root must be rejected");
    assert!(error.to_string().contains("session root cannot change"));
    let session = handle.session("immutable-root").unwrap();
    assert_eq!(session.cwd(), first_root);
    assert_eq!(session.async_fs().root(), first_root);
    assert!(!rejected_root.exists());
}

#[tokio::test]
async fn repeated_remount_rejects_before_copying_files() {
    let directory = tempfile::tempdir().unwrap();
    ...
    let resolver = bind_resolver_fixture(&handle);
    resolver(
        xai_tool_protocol::SessionId::new("single-remount").unwrap(),
        Some(serde_json::json!({"cwd": initial_root})),
    )
    .await
    .expect("initial bind");
    let session = handle.session("single-remount").unwrap();
    session
        .set_cwd_for_virtualization(first_root.clone())
        .await
        .unwrap();
    std::fs::write(first_root.join("private.txt"), "keep in original session").unwrap();
    assert!(
        session
            .set_cwd_for_virtualization(rejected_root.clone())
            .await
            .is_err()
    );
    assert_eq!(session.cwd(), first_root);
