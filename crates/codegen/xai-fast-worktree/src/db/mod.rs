...
impl WorktreeDb {
    ...
    /// Open the default DB at `~/.opengrok/worktrees.db`.
    ///
    /// Discovers grok home via `$OPENGROK_HOME`, falling back to the canonicalized
    /// `$HOME/.opengrok` (matching `xai_grok_config::grok_home`).
    ...
    pub fn get(&self, id_or_path: &str) -> Result<Option<WorktreeRecord>> {
        if id_or_path.contains('/') {
            ...
            let lexical = crate::worktree::plan::canonicalize_for_id(&canon);
            if let Some(record) = queries::get_by_path(&self.conn, &lexical)? {
                return Ok(Some(record));
            }
            let canon = if crate::nfs::dest_is_known_unmounted(&canon) {
                dunce::canonicalize(&canon).unwrap_or(canon)
            } else {
                lexical
            };
...
/// Serializes tests that mutate the process-global `OPENGROK_HOME` env var so they
...
#[cfg(test)]
static OPENGROK_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`OPENGROK_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `OPENGROK_HOME` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `OPENGROK_HOME` before `_lock` releases, so the env is correct
...
impl GrokHomeFixture {
    pub(crate) fn new() -> Self {
        let lock = OPENGROK_HOME_ENV_LOCK
            .lock()
        ...
        // via OPENGROK_HOME, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until OPENGROK_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("OPENGROK_HOME");
        unsafe { std::env::set_var("OPENGROK_HOME", &home) };
        Self {
            _lock: lock,
            prev,
...
impl Drop for GrokHomeFixture {
    fn drop(&mut self) {
        unsafe {
            if self.touched_grove_env {
                for (name, value) in [
                    ("XDG_DATA_HOME", self.prev_xdg_data_home.take()),
                    ("GROVE_DATA_DIR", self.prev_grove_data_dir.take()),
                    ("HOME", self.prev_home.take()),
                ] {
                    match value {
            ...
            match self.prev.take() {
                Some(p) => std::env::set_var("OPENGROK_HOME", p),
                None => std::env::remove_var("OPENGROK_HOME"),
            }
        }
    }
}
