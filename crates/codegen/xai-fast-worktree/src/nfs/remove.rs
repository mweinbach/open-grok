...
fn lookup_from_markers(worktree_path: &Path) -> Option<NfsRemoveMeta> {
    for data in super::liveness::candidate_data_dirs() {
        ...
        for ent in entries.flatten() {
            if !ent.file_type().is_ok_and(|file_type| file_type.is_dir()) {
                continue;
            }
            ...
            let Some(marker) = read_backing_marker(&ent.path()) else {
                continue;
            };
            if marker.worktree_id != dirent {
                continue;
            }
...
pub fn read_backing_marker(backing: &Path) -> Option<BackingMarker> {
    ...
    let file = std::fs::OpenOptions::new()
        ...
        .open(backing.join(BACKING_MARKER_FILE))
        .ok()?;
...
mod tests {
    ...
    fn rm_planted_marker_does_not_delete_victim_backing() {
        ...
        .unwrap();
        let journal = rusqlite::Connection::open(data.join("daemon.db")).unwrap();
        journal.execute_batch("CREATE TABLE wt_create_state (worktree_id TEXT PRIMARY KEY, phase TEXT NOT NULL, dest TEXT NOT NULL, source TEXT NOT NULL, orphan_seen_at INTEGER, updated_at INTEGER NOT NULL)").unwrap();
        journal.execute(
            "INSERT INTO wt_create_state (worktree_id, phase, dest, source, updated_at) VALUES (?1, 'committed', ?2, ?3, 1)",
            rusqlite::params![victim_id, victim_dest.to_string_lossy(), tmp.path().join("repo").to_string_lossy()],
        ).unwrap();
