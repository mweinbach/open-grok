...
pub fn candidate_data_dirs() -> Vec<PathBuf> {
    ...
    if let Some(grok_home) = std::env::var_os("OPENGROK_HOME")
        .map(PathBuf::from)
        .or_else(|| dirs::home_dir().map(|home| home.join(".opengrok")))
    {
...
pub fn gc_orphan_pins(
    ...
    for (id, (source, pin_ref, dest, backing)) in candidates {
        ...
        if dest
            .as_ref()
            .is_some_and(|dest| !super::dest_is_known_unmounted(dest))
        {
            report.kept_live += 1;
            state.orphans.remove(&id);
            continue;
        ...
        match pin_exists(&source, &id) {
            ...
            Err(e) => {
                tracing::warn!(
                    error = %e,
                    id,
                    "pin_exists failed; retaining pin"
                );
...
fn pin_exists(source: &Path, worktree_id: &str) -> Result<bool> {
    if !is_safe_worktree_id(worktree_id) {
        anyhow::bail!("invalid worktree pin id");
    }
    let output = crate::git::checkout::git_command()
        .current_dir(source)
        .args([
            "show-ref",
            "--verify",
            "--quiet",
            &format!("refs/grok/worktrees/{worktree_id}"),
        ])
        .output()?;
...
fn delete_pin_ref_gated(source: &Path, worktree_id: &str) -> Result<()> {
    ...
    let resolved = crate::git::checkout::git_command()
        .current_dir(source)
        .args(["rev-parse", "--verify", &pin])
        .output()?;
    if !resolved.status.success() {
        anyhow::bail!("cannot resolve worktree pin before deletion");
    }
    let expected = String::from_utf8(resolved.stdout)?;
    let output = crate::git::checkout::git_command()
        .current_dir(source)
        .args(["update-ref", "--no-deref", "-d", &pin, expected.trim()])
        .output()?;
    if !output.status.success() {
        anyhow::bail!(
            "cannot delete worktree pin: {}",
            String::from_utf8_lossy(&output.stderr)
        );
    }
    Ok(())
...
mod tests {
    ...
    fn identities_prefer_metadata_grove_over_legacy_nfs() {
        let mut rec = crate::db::WorktreeRecord {
            id: "wt-grove".into(),
            path: "/tmp/wt-grove".into(),
            source_repo: "/tmp/source".into(),
            repo_name: "source".into(),
            kind: crate::db::WorktreeKind::Session,
            creation_mode: "grove-fuse".into(),
            git_ref: None,
            head_commit: None,
            session_id: None,
            creator_pid: None,
    ...
    #[test]
    fn pin_delete_does_not_follow_symbolic_refs() {
        xai_test_utils::require_git!();
        let temporary = TempDir::new().unwrap();
        let repository = temporary.path().join("repository");
        std::fs::create_dir(&repository).unwrap();
        init_git_repo(&repository);
        std::fs::write(repository.join("tracked.txt"), "keep").unwrap();
        git_commit_all(&repository, "initial");
        let expected = git_rev_parse(&repository, "HEAD");
        assert!(
            crate::git::checkout::git_command()
                .current_dir(&repository)
                .args(["update-ref", "refs/heads/preserved", &expected])
                .status()
                .unwrap()
                .success()
        );
        assert!(
            crate::git::checkout::git_command()
                ...
                .args([
                    "symbolic-ref",
                    "refs/grok/worktrees/wt-symbolic",
        ...
        delete_pin_ref_gated(&repository, "wt-symbolic").unwrap();
        assert_eq!(git_rev_parse(&repository, "refs/heads/preserved"), expected);
        assert!(!pin_exists(&repository, "wt-symbolic").unwrap());
    }
}
