...
impl WorkspaceStore {
    ...
    pub fn snapshot(&self) -> Result<WorkspaceSnapshot> {
        ...
        self.snapshot_inner()
            .map_err(|error| classify_busy(error, "snapshot", started))
    }

    fn snapshot_inner(&self) -> Result<WorkspaceSnapshot> {
        let tx = self.conn.unchecked_transaction()?;
        let grouping: String =
            tx.query_row("SELECT grouping FROM meta WHERE id = 0", [], |row| {
                row.get(0)
            })?;
    ...
    pub fn data_version(&self) -> Result<i64> {
        ...
        self.conn
            ...
            .map_err(|error| classify_busy(error, "data_version", started))
    }
    ...
    pub fn insert_member(&mut self, member: NewMember) -> Result<InsertOutcome> {
        ...
        let outcome = self
            ...
            .map_err(|error| classify_busy(error, "insert_member", started))?;
        match &outcome {
    ...
    fn insert_member_inner(
        ...
        let transaction = self.with_write("insert_member", |tx| {
            ...
            let count: i64 = tx.query_row("SELECT COUNT(*) FROM members", [], |row| row.get(0))?;
            ...
            if count >= capacity {
                ...
                let rows = stmt.query_map(rusqlite::named_params! {":excess": excess}, |row| {
                    Ok(EvictedRow {
                        session_id: row.get(0)?,
                        kind: row.get(1)?,
                        last_change_unix_ms: row.get(2)?,
                    })
                })?;
                for row in rows {
                    victims.push(row?);
    ...
    pub fn update_member_metadata(
        ...
        .map_err(|error| classify_busy(error, "update_member_metadata", started))?;
        tracing::trace!(
            session_id = %key.session_id,
            kind = key.kind.as_str(),
            "workspace member metadata updated"
    ...
    pub fn remove_member(&mut self, key: &MemberKey) -> Result<RemoveOutcome> {
        ...
        let changed = self
            ...
            .map_err(|error| classify_busy(error, "remove_member", started))?;
    ...
    fn set_ranks(&mut self, column: RankColumn, assignments: &[RankAssignment]) -> Result<()> {
        ...
        self.set_ranks_inner(column, assignments)
            .map_err(|error| classify_busy(error, column.op_name(), started))?;
        tracing::debug!(
    ...
    pub fn set_grouping(&mut self, grouping: &Grouping) -> Result<()> {
        ...
        .map_err(|error| classify_busy(error, "set_grouping", started))?;
    ...
    pub fn rekey(&mut self, old: &MemberKey, new_session_id: SessionId) -> Result<RekeyOutcome> {
        ...
        let outcome = self
            ...
            .map_err(|error| classify_busy(error, "rekey", started))?;
        tracing::info!(
