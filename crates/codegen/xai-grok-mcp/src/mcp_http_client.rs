...
//! (only connect failures and graceful EOF consult it). We ship rmcp 3.2;
//! still unfixed upstream as of rmcp 3.2.0 (an errored stream re-enters
//! `Retrying { retry_times: 0 }` immediately, bypassing the retry policy):
