...
impl std::fmt::Debug for ElicitationInbox {
    fn fmt(&self, value_f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        value_f
            .debug_struct("ElicitationInbox")
...
pub fn accept_result(content: Option<serde_json::Value>) -> ElicitResult {
    ...
    if let Some(value_c) = content {
        result = result.with_content(value_c);
    }
    result
}
...
mod tests {
    ...
    #[tokio::test]
    async fn closing_inbox_wakes_waiting_receiver() {
        let inbox = ElicitationInbox::new();
        let receiver_inbox = inbox.clone();
        let receiver = tokio::spawn(async move { receiver_inbox.recv().await });
        tokio::task::yield_now().await;
        inbox.close();
        let result = tokio::time::timeout(std::time::Duration::from_secs(1), receiver)
            .await
            .expect("closing must wake the coordinator")
            .expect("receiver task must complete");
