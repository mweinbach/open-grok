...
pub(crate) async fn decide_http_auth_from_disk(server_name: &str, url: &str) -> HttpAuthDecision {
    ...
    .await
    .unwrap_or_else(|value_e| {
        tracing::warn!(server = server_name, %value_e, "stored auth classification task failed");
    ...
    match verdict {
        StoredAuthVerdict::StoredCredentials => {
            match rmcp::transport::auth::AuthorizationManager::new(url).await {
                ...
                Err(value_e) => {
                    tracing::warn!(server = server_name, %value_e, "Failed to create OAuth manager");
