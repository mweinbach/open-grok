...
fn handshake_succeeds_against_ca_loaded_from_ssl_cert_file() {
    ...
    unsafe {
        std::env::remove_var(xai_grok_extra_ca::ENV_OPENGROK_EXTRA_CA_BUNDLE);
        std::env::set_var(xai_grok_extra_ca::ENV_SSL_CERT_FILE, ca_path.as_os_str());
    }

    assert_eq!(
        xai_grok_extra_ca::extra_root_ders(),
        &[ca_cert.der().as_ref().to_vec()]
    );
    assert_eq!(
        xai_grok_extra_ca::configured_bundle_env(),
        Some(xai_grok_extra_ca::ENV_SSL_CERT_FILE)
    );
