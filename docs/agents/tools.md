...
- **Background execution (default):** `run_in_background: true` registers the run as a *virtual task* on the session's terminal backend (`computer/virtual_tasks.rs` decorator, wrapped at `spawn.rs` backend construction) and detaches the driver. The run then has full background-task parity with zero tool changes: `get_command_or_subagent_output` polls it (progress log at `<session>/workflows/<run_id>/progress.log`), `kill_command_or_subagent` and the TUI kill button interrupt it (graceful: cell terminated, children cancelled, partial result + resume hint recorded), and completion fires the standard `TaskCompleted` auto-wake. Follow-up user messages never end a running workflow.
- **Resume controls:** journal keys exclude display-only `label`/`phase` (old journals migrate on load). `resume_mode: "positional"` replays by call position for edited scripts; `resume_through: <phase | label | index>` bounds replay so everything after that point re-runs — the interrupt → edit script → resume loop.
